...
#define USBLOG(l,a)     do { if (l <= USBLOG_LEVEL) uart_printf a; } while (0)
...
int usb_send_control_write(struct usb_device *dev, uint8_t request_type, uint8_t request, uint16_t value, uint16_t index, uint16_t length, uint8_t *data)
{
    ...
    do
    {
        USBLOG(USBLOG_INFO, ("USB: Control Write %02x %02x %04x %04x %04x\n", request_type, request, value, index, length));

        USBLOG(USBLOG_INFO, ("USB: device address = %d\n", dev->address));
...
int usb_send_control_read(struct usb_device *dev, uint8_t request_type, uint8_t request, uint16_t value, uint16_t index, uint16_t length, uint8_t *buffer, uint16_t rx_length)
{
    ...
    do
    {
        USBLOG(USBLOG_INFO, ("USB: Control Read %02x %02x %04x %04x %04x\n", request_type, request, value, index, length));
    ...
    while (res && received < rx_length)
    {
        ...
        if (res < 0) {
            USBLOG(USBLOG_INFO, ("USB: Control IN error, res=%d\n", res));
            break;
        }
        USBLOG(USBLOG_INFO, ("USB: Control IN res=%d\n", res));
...
// usb_get_descriptor:
//-----------------------------------------------------------------
int usb_get_descriptor(struct usb_device *dev, uint16_t value, uint16_t index, uint8_t *buffer, uint16_t length)
{
...
int usb_configure_device(struct usb_device *dev, int device_address)
{
    ...
    // Reset device state
    uart_printf("dev->address = %d\n", dev->address); 
    usb_reset_device(dev);
    uart_printf("after reset: dev->address = %d\n", dev->address); 
    ...
    uart_printf("after get_descriptor: dev->address = %d\n", dev->address); 
    ...
    dev->str_manufacturer = dev_desc->iManufacturer;
    dev->str_product      = dev_desc->iProduct;
    dev->str_serial       = dev_desc->iSerialNumber;
                  ...
                  int (*enum_class_specific)(struct usb_device *dev, struct usb_interface *intp, void *desc))
{
    ...
    for (offset = conf_desc->bLength;(offset < conf_size) && res; )
    {
        ...
        USBLOG(USBLOG_ENUM, ("== Descriptor: Type=%d Len=%d\n", desc_hdr->type, desc_hdr->size));

        uint8_t *data = (uint8_t*)(&_desc_buffer[offset] + sizeof(struct UsbDescriptorHeader));
        ...
        switch (desc_hdr->type)
        {
            case DESC_INTERFACE:
            {
                ...
                dev->interfaces[current_int].if_endpoints = num_endpoints;

                // set current endpoint to 0
                current_endp = 0;
            }
            break;
            ...
            case DESC_ENDPOINT:
            {
                ...
                USBLOG(USBLOG_ENUM, ("Endpoint %d:\n", ep_addr));
                USBLOG(USBLOG_ENUM, (" bEndpointAddress=%02x\n", endp_desc->bEndpointAddress));
                USBLOG(USBLOG_ENUM, (" bmAttributes=%02x\n", endp_desc->bmAttributes));
                USBLOG(USBLOG_ENUM, (" wMaxPacketSize=%d\n",  USB_BYTE_SWAP16(endp_desc->wMaxPacketSize)));
                USBLOG(USBLOG_ENUM, (" bInterval=%d\n", endp_desc->bInterval));

                if (current_int == -1 || current_endp >= MAX_ENDPOINTS)
                {
                    USBLOG(USBLOG_ERR, ("Endpoint Error! current_int=%d\n", current_int));
                    return 0;
                }

                // Store details about endpoint
                dev->interfaces[current_int].endpoint[current_endp].max_packet_size = USB_BYTE_SWAP16(endp_desc->wMaxPacketSize);
                dev->interfaces[current_int].endpoint[current_endp].endpoint_type   = endp_desc->bmAttributes & ENDPOINT_TYPE_MASK;
                dev->interfaces[current_int].endpoint[current_endp].endpoint        = ep_addr;
                dev->interfaces[current_int].endpoint[current_endp].direction       = endp_desc->bEndpointAddress & ENDPOINT_DIR_MASK;  
                dev->interfaces[current_int].endpoint[current_endp].interval        = endp_desc->bInterval;
                current_endp++;
            }
            break;
            ...
            case DESC_IF_POWER:
            {
                USBLOG(USBLOG_ENUM, ("Skipping descriptor %d\n", desc_hdr->type));
            }
            break;
